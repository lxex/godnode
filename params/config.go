...
var (
	// TODOShawn: change MainnetGenesisHash
	MainnetGenesisHash = common.HexToHash("0x85d9dfd6832abb8fc59bac2c3cadea086cd5dc5cf89bcba31ccb563dab72f5e6")
...
var (
	...
	MainnetChainConfig = &ChainConfig{
		...
		IsJiangXiBlock:      big.NewInt(112800),
	...
	AllEthashProtocolChanges = &ChainConfig{big.NewInt(1337), big.NewInt(0), nil, false, big.NewInt(0), common.Hash{}, big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), big.NewInt(0), nil, nil, big.NewInt(112800), new(EthashConfig), nil}
...
type ChainConfig struct {
	...
	IsJiangXiBlock *big.Int `json:"isJiangXiBlock,omitempty"`
...
func (c *ChainConfig) String() string {
	...
	return fmt.Sprintf("{ChainID: %v Homestead: %v DAO: %v DAOSupport: %v EIP150: %v EIP155: %v EIP158: %v Byzantium: %v Constantinople: %v Petersburg: %v Istanbul: %v, Muir Glacier: %v, Berlin: %v, YOLO v3: %v, JiangXi: %v, Engine: %v}",
		c.ChainID,
		c.HomesteadBlock,
		c.DAOForkBlock,
		c.DAOForkSupport,
...
func (c *ChainConfig) CheckConfigForkOrder() error {
	...
	for _, cur := range []fork{
		...
		{name: "isJiangXiBlock", block: c.IsJiangXiBlock},
	} {
		if lastFork.name != "" {
...
func (c *ChainConfig) checkCompatible(newcfg *ChainConfig, head *big.Int) *ConfigCompatError {
	...
	if isForkIncompatible(c.IsJiangXiBlock, newcfg.IsJiangXiBlock, head) {
		return newCompatError("JiangXi fork block", c.IsJiangXiBlock, newcfg.IsJiangXiBlock)
	}
	return nil
}
